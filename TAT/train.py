...
def train_model(model, dataloaders, args, logger):
    ...
    log_dir = Path(args.log_dir) / 'tb_logs' / args.dataset / '_'.join(
        [args.time_str, args.desc.replace(' ', '_')])  # add desc after time, as log dir name
    ...
    for step in range(args.epoch):
        optimize_model(model, target_model, train_loader, optimizer, logger, summary_writer, step,
                       perm_loss=args.perm_loss)

        train_metrics_results = eval_model(model, train_loader, metrics, desc='train_eval',
                                           start_step=step * len(train_loader))
        ...
        test_metrics_results = eval_model(model, test_loader, metrics, desc='test_eval',
                                          start_step=step * len(test_loader))
...
def label_to_order(label):
    ...
    return label_to_order_dict[label]
...
def bleu_metric(predictions, labels):
    with warnings.catch_warnings():
        ...
        for i in range(len(labels)):
            reference = [label_to_order(labels[i])]
            candidate = label_to_order(predictions[i])
...
def permute_optimize(model, prediction, target_model, batch, optimizer):
    ...
    for i in range(num_graphs):  # align
        permuted_prediction[i] = permuted_prediction[i][
            inverse_permute_matrix[:, pemu_rand[i]]]  # aligned with prediction
