...
def train_model(model, dataloaders, args, logger):
    ...
    for step in range(args.epoch):
        optimize_model(model, target_model, train_loader, optimizer, logger, summary_writer, step,
                       perm_loss=args.perm_loss, set_indice_length=args.set_indice_length, timestep=args.timestep)
        train_metrics_results = eval_model(model, train_loader, metrics, desc='train_eval', start_step=step*len(train_loader),
                                           set_indice_length=args.set_indice_length, timestep=args.timestep)
...
def eval_model(model, dataloader, metrics, **kwargs):
    ...
    if timestep == -1:
        timestep = list(range(kwargs["set_indice_length"]))
    else:
        timestep = [timestep]
        ...
        for i, batch in tqdm(enumerate(dataloader), total=len(dataloader), desc=desc):
            try:
                ...
                for ts in timestep:
                    ...
                    label_seq_t = label_seq[:, ts]
                    label_seq_t_expanded = torch.index_select(
                        lookup_table_n(kwargs["set_indice_length"], return_tensor=True),
                        0, label_seq_t).to(torch.float32)
                    prediction = model(batch_copy, ts)
                    curr_prediction.append(prediction)
                    curr_label.append(label_seq_t_expanded)
                predictions.append(torch.cat(curr_prediction, dim=1))
                labels.append(torch.cat(curr_label, dim=1))
            except RuntimeError:
                continue
    ...
    metrics_results = compute_metric(predictions, labels, metrics, n=kwargs["set_indice_length"])
    return metrics_results


def acc_metric(predictions, labels, num_nodes=3):
    new_predictions = []
    for i in range(0, predictions.shape[1], num_nodes):
        _, idx = predictions[:, i:i + num_nodes].topk(2, dim=1)
        new_predictions.append(torch.nn.functional.one_hot(idx, num_nodes).sum(dim=1))
    predictions = torch.cat(new_predictions, dim=1)
...
def compute_metric(predictions, labels, metrics, n=3):
    metrics_results = {}
    for key, f in metrics.items():
        if key == 'acc':
            metrics_results[key] = f(predictions, labels, num_nodes=n)
        else:
    ...
    for i, batch in tqdm(enumerate(dataloader), total=len(dataloader), desc='train'):
        try:
            ...
            for ts in timestep:
                batch_copy = copy.deepcopy(batch)
                label_seq_t = label_seq[:, ts]
                label_seq_t_expanded = torch.index_select(lookup_table_n(kwargs["set_indice_length"], return_tensor=True),
                                                          0, label_seq_t).to(torch.float32)
                prediction = model(batch_copy, ts)
                loss = criterion(prediction, label_seq_t_expanded)
                loss.backward()
